...
namespace Blam.Cache
{
    ...
    public class TagSerializer
	{
		...
		public TagSerializer(GameVersion version)
		{
			_version = version;
		}
		...
		public void Serialize(ISerializationContext context, object tagStructure)
		{
			...
			var info = new TagDefinition(tagStructure.GetType(), _version);
		...
		private void SerializeStruct(ISerializationContext context, MemoryStream tagStream, IDataBlock block, TagDefinition info, object structure)
		{
		...
		private void SerializeComplexValue(ISerializationContext context, MemoryStream tagStream, IDataBlock block, object val, TagFieldAttribute valueInfo, Type valueType)
		{
			if (valueInfo != null && ((valueInfo.Flags & TagFieldFlags.Indirect) != 0 || valueType == typeof(ResourceReference)))
				...
				SerializeStruct(context, tagStream, block, new TagDefinition(val.GetType(), _version), val);
		}
